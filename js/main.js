...
let youLoseSoundEffect = document.querySelector(".lose-sound");
let youWinSoundEffect = document.querySelector(".win-sound");
...
function generateRandomMatrix() {
  //generate ramdom matrix from given 12 matrixes;
  matMatrix = [
    mat1,
    mat2,
    mat3,
    ...
    mat11,
    mat12,
  ]; // we added the 12 matss to an array
...
function difficulty(percentage) {
  ...
  for (i = 0; i < percentage; i++) {
    ...
    if (!document.getElementById(cellName).innerHTML == "") {
      // if cell is not empty בדיקה האם התאים ריקים ממחרוזת
...
function createResetMat() {
  // יצירת אתחול למטריצה

  for (i = 0; i < allCells.length; ) {
    // we loop through the cells לולאה שעוברת על כל התאים
    for (j = 0; j < 9; j++) {
      // לולאה שעוברת על כל השורות
      for (k = 0; k < 9; k++) {
...
// switch levels
const swichLevel = () => {
  // פונקציה שמחזירה את המשתמש לדף השני
...
const logIn = (user, pass) => {
  ...
  if (user == username && pass == password) {
    // check for match בדיקת התאמה בין שם המשתמש והסיסמא
    // if there is a match
    pageDisplay("none", "block", "none");
  } else {
    errorPageOne.innerHTML = "Username and Password doe's not match"; //else we put a message that there is no match // אם אין התאמה בין השם משתמש והסיסמא אז תהיה הודעה שאומרת את זה
  }
};
...
function finishGame() {
        ...
        if (j == 1 || j == 4 || j == 7) {
          ...
          for (k = 0; k < 9; k++) {
            if (islandCheck[k] == "" || islandCheck[k] == islandCheck[k + 1]) {
              console.log("LOSE");
              youLostDiv.style.display = "block";
              youLoseSoundEffect.display = "block";
              return false;
            }
          }
        }
  ...
  for (i = 0; i < 9; i++) {
    ...
    for (j = 0; j < 9; j++) {
      if (mat[i][j] == "" || mat[i][j] == mat[i][j + 1]) {
        console.log("LOSE");
        youLostDiv.style.display = "block";
        youLoseSoundEffect.style.display = "block";
        youLoseSoundEffect.classList.add("lose-sound");
  ...
  youWinSoundEffect.style.display = "block";
  youWinSoundEffect.classList.add("win-sound");
  console.log("DONE");
  return true;
}
...
for (i = 0; i < levelsBtn.length; i++) {
  levelsBtn[i].addEventListener("click", function () {
    if (this.textContent == "Easy") {
      //if this button is clicked we delete 0.28 cells from board randomly
      generateForLevelChange(mat, 1);
      listenersDisplayReset();
    } else if (this.textContent == "Medium") {
      //if this button is clicked we delete 0.45 cells from board randomly
      generateForLevelChange(mat, 36);
      listenersDisplayReset();
    } else if (this.textContent == "Hard") {
      //if this button is clicked we delete 0.65 cells from board randomly
      generateForLevelChange(mat, 52);
      listenersDisplayReset();
    } else {
      //else we go insane we delete 0.87 cells from board randomly
